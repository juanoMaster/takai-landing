# CLAUDE.md — takai-landing (takai.cl)
...
- **Tailwind SÍ se usa en esta landing** (a diferencia de `owner-dashboard`, donde está prohibido). El rediseño premium de 2026-07 migró todo el sitio a Tailwind con la paleta de marca (`crema`, `tinta`, `cobre`, `ceniza`, `humo`) definida en `tailwind.config.ts`; usa esas clases y esos tokens, no colores sueltos. Excepción viva: `ContactModal.tsx` sigue con inline styles — no hace falta migrarlo. Clases CSS globales sueltas solo en string `<style>` con prefijo `tk-`.
