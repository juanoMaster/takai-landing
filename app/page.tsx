...
// Calendar color helpers — outside JSX, no non-null assertions
function calBg(r: string) {
  if (r === "green") return "rgba(40,140,60,0.15)"
  if (r === "blue") return "rgba(30,100,200,0.15)"
  if (r === "orange") return "rgba(200,110,20,0.15)"
  return "transparent"
}
function calBd(r: string) {
  if (r === "green") return "rgba(60,180,80,0.25)"
  if (r === "blue") return "rgba(50,140,240,0.25)"
  if (r === "orange") return "rgba(220,130,30,0.25)"
  return "transparent"
}
function calTc(r: string) {
  if (r === "green") return "#4ab870"
  if (r === "blue") return "#5a9ad4"
  if (r === "orange") return "#d4a84a"
  return "#1e3820"
}

interface CalCell { d: number | null; res?: string; first?: boolean; last?: boolean; label?: string }

function CalendarMockup() {
  const cells: CalCell[] = [
    {d:null},{d:null},{d:null},{d:null},{d:null},{d:1},{d:2},
    {d:3},{d:4},{d:5,res:"green",first:true,label:"Fam. Sepulveda"},{d:6,res:"green"},{d:7,res:"green"},{d:8,res:"green",last:true},{d:9},
  ...
  const legends = [
    { c: "green", l: "Confirmada" },
    { c: "blue", l: "Manual" },
    { c: "orange", l: "Pendiente" },
  ]
  return (
    <div style={{ background: "#070e09", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "22px 20px", width: "100%", maxWidth: "420px", boxShadow: "0 40px 80px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03)" }}>
          ...
          {legends.map(function(item) {
            return (
              <div key={item.c} style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                <div style={{ width: "7px", height: "7px", borderRadius: "2px", background: calTc(item.c) }} />
                <span style={{ fontSize: "9px", color: "#1a3020" }}>{item.l}</span>
              </div>
            )
          })}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "3px" }}>
        {cells.map(function(cell, i) {
          ...
          const br = cell.first ? "6px 0 0 6px" : cell.last ? "0 6px 6px 0" : hasBg ? "0" : "4px"
          return (
            <div key={i} style={{ height: "38px", background: hasBg ? calBg(r) : "transparent", border: "1px solid " + (hasBg ? calBd(r) : "transparent"), borderRadius: br, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", gap: "1px" }}>
              {cell.d !== null && (
                <span style={{ fontSize: "11px", color: hasBg ? calTc(r) : "#1e3820", fontWeight: hasBg ? 700 : 400, lineHeight: 1 }}>
                  {cell.d}
                </span>
              )}
              {cell.first && cell.label && (
                <span style={{ fontSize: "7px", color: calTc(r), whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "95%", padding: "0 1px", lineHeight: 1, opacity: 0.85 }}>
                  {cell.label}
                </span>
              )}
            </div>
